                            ...
                            <div class="card-body">
                                ...
                                <div class="form-row">
                                    <div class="form-group col-lg-6">
                                        ...
                                        <input id="name" type="text" name="name" class="form-control {{ $errors->has('name') ? " is-invalid " : '' }}" value="{{ old('name',$tag->name) }}">
                                        @error('name')
                                    ...
                                    <div class="form-group col-lg-6">
                                        ...
                                        <select id="status" name="status" class="form-control @error('status') is-invalid @enderror">
                                            @foreach(trans('statuses') as $key => $status)
                                                <option value="{{ $key }}" {{ (old('status') == $key || $tag->status == $key) ? 'selected' : '' }}>{{ $status }}</option>
                                            @endforeach
                                        </select>
                                        @error('status')
                                        <div class="invalid-feedback">
